...
### Requirement: Recommendation Access Matrix

The capability `view_recommendations` MUST be ratified with admin ✅, psicólogo ✅, and evaluado ✅ for own sessions only; both recommendation routes MUST declare `require_roles(...)` (deny-by-default). An evaluado accessing another user's session MUST receive `FORBIDDEN` with no data exposure and an `auth.denied` audit record. The capability MUST be updated in lockstep across `permissions.py`, the contracts README §6 matrix, and the capability contract tests.

#### Scenario: Owner reads own recommendations

- GIVEN an `evaluado` user with a generated session
- WHEN the recommendations are read
- THEN 200 returns the aggregated payload

#### Scenario: Foreign evaluado denied

- GIVEN an `evaluado` user and another user's generated session
- WHEN the recommendations are read or generated
- THEN `FORBIDDEN` is returned
- AND no recommendation data is exposed and `auth.denied` is audited

### Requirement: Recommendation Availability Errors

The recommendation surface MUST return `NOT_FOUND` with message `resource_not_found` for a missing session, for a completed-but-unscored session (generate), and for a session with no generation (read) — indistinguishable responses that reveal no existence or state. The generation trigger on a session that is not `completed` MUST return `CONFLICT` with message `session_not_completed` and MUST NOT expose response or score data. All recommendation errors MUST use the single envelope (code, message, request_id, details).
...
- WHEN each is requested via the recommendation surface
- THEN all return the identical `NOT_FOUND`/`resource_not_found` envelope

#### Scenario: In-progress generation triggers stable conflict
