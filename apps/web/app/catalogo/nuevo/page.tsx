...
export default function NewInstrumentPage() {
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    if (busy) return;
    if (!/^[A-Z0-9_.-]+$/.test(key)) {
      setError("La clave solo admite mayúsculas, números, punto, guion bajo y guion medio.");
      return;
    }
    setBusy(true);
    setError(null);
    try {
      const created = await apiFetch<CreateResponse>("/api/v1/catalog/admin/instruments", {
        method: "POST",
      ...
      router.push(`/catalogo/${created.instrument.id}/versiones/${created.draft.instrument_version_id}`);
    } catch (err) {
      setError(err instanceof ApiError ? err.payload.message : "No se pudo crear el instrumento.");
      setBusy(false);
    }
  }

  if (!canManage) {
    return <p style={{ fontFamily: "system-ui, sans-serif", padding: "2rem" }}>No tiene permisos para esta sección.</p>;
  }
    ...
    <main style={{ fontFamily: "system-ui, sans-serif", maxWidth: 640, margin: "2rem auto", padding: "0 1rem" }}>
      ...
      <form onSubmit={onSubmit} style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem" }}>
          ...
          <button type="button" onClick={() => router.push("/catalogo")} style={{ padding: "0.6rem", cursor: "pointer" }}>
            Cancelar
          </button>
        </div>
      </form>
