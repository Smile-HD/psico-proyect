...
export default function PublishedViewPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    apiFetch<PublishedVersion>(`/api/v1/catalog/published-versions/${params.versionId}`, {
      ...
      .catch((err) => {
        if (!cancelled) {
          setError(err instanceof ApiError ? err.payload.message : "No se pudo cargar la versión publicada.");
        }
      });
    return () => {
      cancelled = true;
    ...
    <main style={{ fontFamily: "system-ui, sans-serif", maxWidth: 760, margin: "2rem auto", padding: "0 1rem" }}>
      <header>
        ...
        <p style={{ margin: 0, color: "#666" }}>
          ...
          {version.published_at ? new Date(version.published_at).toLocaleString("es-ES") : ""}
        </p>
      ...
      {version.scales.map((scale) => (
        <section key={scale.id} style={{ marginTop: "1.5rem" }}>
          ...
          {scale.items.map((item) => (
            <div key={item.id} style={{ marginBottom: "1rem", padding: "0.75rem", border: "1px solid #eee", borderRadius: 4 }}>
              <p style={{ margin: 0, fontWeight: item.required ? 600 : 400 }}>
                {item.item_order}. {item.text}
                {item.required ? <span style={{ color: "#b3261e" }}> *</span> : null}
              </p>
      ...
      <footer style={{ marginTop: "2.5rem" }}>
        <Link href={`/catalogo/${params.instrumentId}/versiones/${params.versionId}`}>← Volver al detalle</Link>
        <p style={{ color: "#666", fontSize: "0.85rem", marginTop: "1rem" }}>
          Contenido sintético y de uso exclusivo para investigación (research-only).
        </p>
      </footer>
    </main>
  );
